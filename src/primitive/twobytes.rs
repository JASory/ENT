...
impl NumberTheory for u16 {
    ...
    fn nth_prime(&self) -> NTResult<Self> {
        ...
        if *self < 54{
          return (*self as u8).nth_prime().map(|y| y as u16)
        }
    ...
    fn radical(&self) -> NTResult<Self> {
     if self.reducible(){
        return (*self as u8).radical().map(|kishum| kishum as u16)
     }
    ...
    fn k_free(&self, k: &Self) -> bool {
       if self.reducible(){
         return (*self as u8).k_free(&(*k as u8))
       }
    ...
    fn euler_totient(&self) -> Self {
       if self.reducible(){
         return (*self as u8).euler_totient() as u16
       }
        let factors = self.factor();
   ...
   fn quadratic_residue(&self, n: &Self) -> Self {
    ...
    fn checked_quadratic_residue(&self, n: &Self) -> NTResult<Self> {
        ...
        NTResult::Eval(((*self as u32 * *self as u32) % *n as u32) as Self)
    }
    ...
    fn checked_product_residue(&self, other: &Self, n: &Self) -> NTResult<Self> {
        ...
        NTResult::Eval(((*self as u32 * *other as u32) % *n as u32) as Self)
    }
