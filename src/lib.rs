/*!
A  library for elementary number theory and arbitrary-precision arithmetic.Intended to be simple,  versatile, performant and above all else
 correct.
...
* Extremely fast and rigourously proven primality checking for integers under 2^64+2^47.
 ...
 * API will break without warning. Number-theory is not even close to a stable release it is simply published to make it available given the fact
  that it surpasses many similar libraries. (Indeed number-theory was originally a subfile to a still private computer algebra project, and
  consequently it's functionality strongly reflects this).
