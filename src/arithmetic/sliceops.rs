...
pub(crate) fn add_slice(x: &mut [u64], y: &[u64]) -> u8 {
    ...
    if carry > 0u8 {
        // if carry is greater than zero, propagate it through the rest of the array until there is no carry left

        for k in hi.iter_mut() {
            //add the carry until there is no carry
