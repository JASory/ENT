 ...
 fn quadratic_residue(&self, n: &Self) -> Self{
     Mpz::from_u128(*self).u_quadratic_residue(&Mpz::from_u128(*n)).to_u128().unwrap()
 }
 ...
 fn mod_pow(&self, p: &Self, modulus: &Self)-> Self{  
     Mpz::from_u128(*self).u_mod_pow(&Mpz::from_u128(*p),&Mpz::from_u128(*modulus) ).to_u128().unwrap()
  }
}  
